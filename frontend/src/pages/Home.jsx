...
export default function Home() {
  ...
  const Badge = ({ status }) => (
    <span className={`text-xs px-2 py-0.5 rounded-full border ${status==='approved' ? 'bg-violet-50 text-violet-700 border-violet-200' : status==='pending' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>{status}</span>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {announcements.length > 0 && (
        <div className="rounded-xl p-3 bg-violet-50 dark:bg-violet-900/30 border border-violet-200 dark:border-violet-800">
          <div className="font-semibold mb-1">Live announcements</div>
          <ul className="text-sm text-violet-800 dark:text-violet-200 list-disc pl-5 space-y-1">
    ...
    <ul className="space-y-2">
      {rows.map((r, idx) => (
        <li key={r._id || idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-800">
          ...
          <span className="text-violet-700 dark:text-violet-300 font-semibold">{r.points} pts</span>
        </li>
      ))}
    </ul>
  );
